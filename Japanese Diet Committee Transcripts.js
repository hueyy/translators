...
NDL.prototype.setCol = function ( pos ) {
	res = ZU.xpath(this.newDoc, '//tr/th[1][contains(text(),"No")]/following-sibling::th['+pos+']')[0];
	if (res) {
...
function doWeb(doc, url) {
	if (detectWeb(doc, url) == "multiple") {
		...
		Zotero.selectItems(items_select, function(items){
			...
			Zotero.debug("PUshing to processDocuments: " + JSON.stringify(urls, null, 2))
	Zotero.Utilities.processDocuments(urls, function(newDoc, url) {
		Zotero.debug("processDocuments OK");
		...
		Zotero.debug("Speaker: "+speaker);

		Zotero.debug("processDocuments OK2");
		Zotero.debug("second-tier data: " + JSON.stringify([items_data[pos]['text_url']], null, 2));
		ZU.processDocuments(
			[items_data[pos]['text_url']],
			function (doc, url) {
				Zotero.debug("second-tier processDocuments OK");
