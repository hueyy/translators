{
	...
	"translatorType": 4,
	"browserSupport": "gcsv",
	"lastUpdated": "2018-05-31 08:05:24"
}
...
NDL.prototype.setCol = function ( pos ) {
	res = ZU.xpath(this.doc, '//tr/th[1][contains(text(),"No")]/following-sibling::th['+pos+']')[0];
	if (res) {
    ...
    ZU.processDocuments(
        ...
        function(doc, url){
		    ...
		    if ( match = speakers_url.match(/.*DPOS=([0-9]+).*/,'\1') ) {
			    session_list_pos = padding( match[1] );
		    } else {
            ...
            for (var anchor of anchorNodes) {
			    ...
			    items_data[pos].text_url = text_url_base + link_info.params + "&PPOS=" + pos;
			    items_data[pos].speaker = anchor.textContent.replace(/.*]/,"");
                items_data[pos].statement_pos = pos;

                items_select[pos] = anchor.textContent;
            }
            Zotero.selectItems(items_select, function(items){
		        ...
		        for (var i in items) {
		            ...
		            var index_url = speakers_url.replace(/dispdoc_speaker/, "list").replace(/MODE=[0-9]+/, "MODE=") + "&MYPOS=" + i;
			        urls.push(index_url);
		        }
	            ZU.processDocuments(
                    urls,
                    function(newDoc, url) {
                        ...
                        var speaker = items_data[pos].speaker;
